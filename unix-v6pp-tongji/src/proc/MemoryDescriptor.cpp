...
{
	User &u = Kernel::Instance().GetUser();
    //首先将和m_UserPageTableArray有关的语句均注释掉
	// if (u.u_MemoryDescriptor.m_UserPageTableArray == NULL)
	// 	return;
	PageTable *pUserPageTable = Machine::Instance().GetUserPageTableArray();//获取#202、#203表
	...
	// 计算代码段、数据段和堆栈段的长度
	unsigned int text_length = (this->m_TextSize + (PageManager::PAGE_SIZE - 1)) /PageManager::PAGE_SIZE;
	unsigned int data_length = (this->m_DataSize + (PageManager::PAGE_SIZE - 1)) /PageManager::PAGE_SIZE;
	unsigned int stack_length = (this->m_StackSize + (PageManager::PAGE_SIZE - 1)) / PageManager::PAGE_SIZE;
	
	unsigned int data_count = 0;// 用来记录数据段页框数量
	...
	for (unsigned int i = 0; i < Machine::USER_PAGE_TABLE_CNT; i++)
	{
		for (unsigned int j = 0; j < PageTable::ENTRY_CNT_PER_PAGETABLE; j++)
		{
			...
			if (i == 1)
			{
				...
				if (j >= 1 && j <= text_length) //代码段刷新
				{
					pUserPageTable[i].m_Entrys[j].m_Present = 1;
					pUserPageTable[i].m_Entrys[j].m_ReadWriter = 0;
					pUserPageTable[i].m_Entrys[j].m_PageBaseAddress = j - 1  + (textAddress >> 12);
				...
				else if (j > text_length && j <= text_length + data_length)//数据段刷新
				{
					pUserPageTable[i].m_Entrys[j].m_Present = 1;					
					pUserPageTable[i].m_Entrys[j].m_ReadWriter = 1;
					pUserPageTable[i].m_Entrys[j].m_PageBaseAddress = (data_count++) + 1+ (u.u_procp->p_addr >> 12);//加一是因为数据段和p_addr需要偏移1个页框号（有一个核心） 
					// pUserPageTable[i].m_Entrys[j].m_ReadWriter = this->m_UserPageTableArray[i].m_Entrys[j].m_ReadWriter;
				...
				else if (j >= PageTable::ENTRY_CNT_PER_PAGETABLE - stack_length) //堆栈段刷新（从最后往前算）
				{
					pUserPageTable[i].m_Entrys[j].m_Present = 1;
					pUserPageTable[i].m_Entrys[j].m_ReadWriter = 1;
					pUserPageTable[i].m_Entrys[j].m_PageBaseAddress = (data_count++) + 1 + (u.u_procp->p_addr >> 12);//加一是因为数据段和p_addr需要偏移1个页框号 
				}
			}
		}
	}
